...
//
// Mount a container-filesystem. Changes to the filesystem will be written to CHANGESDIR.
...
#define CHANGESDIR_ALLOWED_CHARS "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789./-_"
...
char * prepare_lowerdir(char* image_id){
    ...
    if (strlen(nodepath) < offset){
        pl_fatal("Unexpected interal error: nodepath command output is shorter than data command output");
    }
    char *curr;
    ...
    while (curr = strsep(&str, "/")){
        asprintf(
                &lowerdir,
                "%s/index/%s/_data/root:%s",
                plash_data,
                curr,
                lowerdir) != -1 || pl_fatal("asprintf");
    }
...
int main(int argc, char *argv[]) {
    ...
    if (changesdir != NULL){
        ...
        asprintf(
                ...
                prepare_lowerdir(image_id),
                prepare_workdir(changesdir),
                prepare_datadir(changesdir)
                ) != -1 || pl_fatal("asprintf");
    } else {
        asprintf(
                &mount_opts,
    ...
    execvp("mount", (char*[]){"mount", "-t", "overlay", "overlay", "-o", mount_opts, mountpoint, NULL});
    pl_fatal("execvp");
}
