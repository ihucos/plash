...
@register_macro()
def hook_after(function, *script):
    orig_func = state['macros'][function]
    @join_result
    def wrapper(*args):
        yield orig_func(*args)
        yield eval([list(script)])
    state['macros'][function] = wrapper


@register_macro()
def hook_before(hook_name, function, macro, *macroargs):
    state['macros']['hook-' + hook_name] = state['macros'][function]
    def wrapper(*args):
        return eval([[macro] + list(macroargs), ['hook-' + hook_name]])
    state['macros'][function] = wrapper


@register_macro()
def hook_delete(hook_name):
    del state['macros']['hook-' + hook_name]
